"""Import and normalize the complete Redpill dice set without cropping its artwork."""
...
def save_face(face: Image.Image, die: str, value: int, root: Path) -> None:
    png_dir = root / "public" / "dice" / "source" / "redpill-compact" / die
    raw_dir = root / "public" / "dice" / "raw" / "redpill" / die
...
def save_preview(faces: dict[tuple[str, int], Image.Image], path: Path) -> None:
    columns = 10
    cell_width = 112
    cell_height = 126
    rows = sum((count + columns - 1) // columns for count in EXPECTED.values())
    preview = Image.new("RGBA", (columns * cell_width, rows * cell_height), (12, 5, 7, 255))
...
def main() -> None:
    ...
    archive = args.root / "source-art" / "redpill"
    ...
    for (die, value), path in parsed.items():
        ...
        save_face(face, die, value, args.root)
        faces[(die, value)] = face

    if args.preview:
        save_preview(faces, args.preview)
    print("Importadas 60 faces Redpill em PNG/RGBA 100x100, sem corte e com proporção preservada.")
